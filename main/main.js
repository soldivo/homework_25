...
function setUserInfoCookie(key, value) {
    ...
    const cookieString = `userInfo=${encoded}; expiers=${date.toUTCString()}; path=/`;
    document.cookie = cookieString;
    console.log(`Cookie: ${cookieString}`);
    document.write(`Cookie: ${cookieString}`);
}
...
function saveUserInfo(key, value) {
    ...
    document.write(`<br> <br> Saved ${key}: ${value}`);
    console.log(`Saved ${key}: ${value}`);
}

function getUserInfo(key) {
    if (sessionStorage.getItem(key) === null) return `key is undefined`;
    document.write(`<br> <br> Retrieved key: ${sessionStorage.getItem(key)}`);
    return `Retrieved key: ${sessionStorage.getItem(key)}`;
}
